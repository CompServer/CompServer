...
urlpatterns = [
    ...
    path("tournament/<int:tournament_id>/", RedirectView.as_view(url='competition'), name="competition_score"), # legacy so they can be hyperlinked to
    path("tournament/<int:tournament_id>/", RedirectView.as_view(url='competition'), name="competition_score_page"), # legacy so they can be hyperlinked to
