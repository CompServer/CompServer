...
class JudgingTests(TestCase):
    ...
    def test_only_competitors_can_advance(self):
        ...
        response = self.admin_client.post("/match/""/judge/", {"advancers": self.__class__.team_no_competition.id})
        self.assertNotEqual(response.status_code, 200, "Shouldn't have been able to advance a team that wasn't even in the match")
