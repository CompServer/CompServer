...
class JudgingTests(TestCase):
    ...
    def test_prereqs_satisfied_before_judge_allowed(self):
        ...
        # self.assertNotEqual(response.status_code, 200, "Shouldn't have been able to advance a team from a previously undetermined match")
        ...
        # response = self.admin_client.post(url, {"advancers": self.__class__.team2_in_competition_and_tournament_and_match.id})
        # self.assertNotEqual(response.status_code, 200, "Shouldn't have been able to advance a team when their competitor is an undetermined match")
        # response = self.admin_client.post(url, {"advancers": self.__class__.team3_in_competition_and_tournament_and_match.id})
        # self.assertNotEqual(response.status_code, 200, "Shouldn't have been able to advance a team from a previously undetermined match")
        # response = self.admin_client.post(url, {"advancers": self.__class__.team4_in_competition_and_tournament_and_match.id})
        # self.assertNotEqual(response.status_code, 200, "Shouldn't have been able to advance a team from a previously undetermined match")
    ...
    def test_only_competitors_can_advance(self):
        ...
        # self.assertNotEqual(response.status_code, 200, "Shouldn't have been able to advance a team that wasn't even in the competition")
        # response = self.admin_client.post(url, {"advancers": self.__class__.team_in_competition_but_no_tournament.id})
        # self.assertNotEqual(response.status_code, 200, "Shouldn't have been able to advance a team that wasn't even in the tournament")
        # response = self.admin_client.post(url, {"advancers": self.__class__.team_in_competition_and_tournament_but_no_match.id})
        # self.assertNotEqual(response.status_code, 200, "Shouldn't have been able to advance a team that wasn't even competing in the match")
        # response = self.admin_client.post(url, {"advancers": self.__class__.team1_in_competition_and_tournament_and_match.id})
        # self.assertEqual(response.status_code, 200, "Should been able to advance the first team in the match")
        # response = self.admin_client.post(url, {"advancers": self.__class__.team2_in_competition_and_tournament_and_match.id})
        # self.assertEqual(response.status_code, 200, "Should been able to advance the second team in the match")
